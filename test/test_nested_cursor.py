...
class NestedTest(_Fixture, fixtures.TablesTest):
    ...
    def test_nested_row(self):
        ...
        sub_stmt = nested(select([order]).where(order.c.customer_id == customer.c.id)).label('o')
        ...
        eq_(
            list(sub_result),
            [(101, 1, 'apple related'), (102, 1, 'apple related'), (103, 1, 'apple related')]
        )
    ...
    def test_double_nested_row(self):
        ...
        sub_sub_stmt = nested(select([item]).where(item.c.order_id == order.c.id)).label('i')
        sub_stmt = nested(select([sub_sub_stmt]).where(order.c.customer_id == customer.c.id)).label('o')
        stmt = select([sub_stmt]).where(customer.c.id == 1)
